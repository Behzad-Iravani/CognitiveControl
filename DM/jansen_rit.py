...
def _numba_dfun_jr(y, c,
    ...
    ##--------------------------------------BEHZAD CHANGED THIS --------------------------------------------
    sigm_y1_y2 = 2.0 * nu_max / (1.0 + math.exp((r * (v0 - (y[1] - y[2])))[0]))
    sigm_y0_1 = 2.0 * nu_max / (1.0 + math.exp((r * (v0 - (a_1 * J * y[0])))[0]))
    sigm_y0_3 = 2.0 * nu_max / (1.0 + math.exp((r * (v0 - (a_3 * J * y[0])))[0]))
    dx[0] = y[3]
    dx[1] = y[4]
    dx[2] = y[5]
    dx[3] = (A * a * sigm_y1_y2 - 2.0 * a * y[3] - a ** 2 * y[0])[0]
    dx[4] = (A * a * (mu + a_2 * J * sigm_y0_1 + c + src) - 2.0 * a * y[4] - a ** 2 * \
            y[1])[0]
    dx[5] = (B * b * (a_4 * J * sigm_y0_3) - 2.0 * b * y[5] - b ** 2 * y[2])[0]
